        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black-950 bg-opacity-60">
            <div className="text-white text-xl flex flex-col items-center justify-between">
                <div className="invisible">
                    <div className="mb-2 max-w-[80%] text-center">{`We're running on free resources, so the first load needs to wake up the service and might take a few minutes.`}</div>
                ...
                <div className={`${(!showFreeResources || !isLoading) ? "invisible" : ""} mt-2 max-w-[80%] text-center`}>{`We're running on free resources, so the first load needs to wake up the service and might take a few minutes.`}</div>
            </div>
        </div>
    );
};
